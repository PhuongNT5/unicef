import './bls_modal.scss';

import {
  convertNodeListToArray, getScrollbarWidth, ajaxRequest, loadTemplate, processTemplate, getSelectors,
} from '../../utils';

const _classModalBody = 'bls-modal__body';
const _classModalHeader = 'bls-modal__header';
const _classClose = 'bls-btn__close';
...
function _loaded(instance) {
  instance.elements.modalContent.classList.add('loading-fade--out');
  setTimeout(() => {
    instance.elements.modalContent.classList.remove('loading-fade--out');
...
function _loadAjax(instance) {
    ...
    try {
      ...
      if (instance.setting.template) {
        ...
        if (template !== '') {
          ...
          if (instance.setting.display !== 'fullscreen') instance.elements.modalContent.classList.add('resizing');
  ...
  if (instance.setting.contentTarget === null) {
    instance.elements.modalContent.querySelector(`.${_classModalBody}`).innerHTML = '';
    instance.elements.modalContent.querySelector(`.${_classModalBody}`).appendChild(ajaxEl);
  } else {
    instance.elements.modalContent.querySelector(`.${instance.setting.contentTarget}`).innerHTML = '';
...
function _buildTargetUI(instance, index) {
  ...
  if (!clone.elements.modal) {
    ...
    modal.classList.add(
      'bls-modal',
      `bls-modal-${index}`,
      `bls-modal--${clone.setting.type}`,
    );
    if (clone.setting.scrollOverlay) modal.classList.add('bls-modal--scroll');
    const modalContent = document.createElement('div');
    modalContent.classList.add('bls-modal__instance');
...
const _closeOverlayHandle = (instance, e) => {
  if (instance.setting.overlay) {
    if (e.target === instance.elements.modal) {
      e.preventDefault();
      e.stopPropagation();
...
function _bindMethodClickOpen(instance) {
  if (window.blsModal !== undefined) {
    window.blsModal.map((x) => {
...
function _buildEvents(instance) {
  ...
  // instance.elements.modal.addEventListener('click', (e) => {
  //   e.preventDefault();
  //   e.stopPropagation();
  ...
  // Click on close buttons (any element has class bls-close in modal)
  // instance.elements.close.map((close) => {
  //   close.addEventListener('click', (e) => {
  //     e.preventDefault();
  //     e.stopPropagation();

  //     if (e.target === close) _closeModal(instance);
  //   }, false);
  //   return close;
  // });

  // After modal animation ended (both open or close event)
  // instance.elements.modal.addEventListener('transitionend', (e) => {
  //   e.preventDefault();
  //   e.stopPropagation();
  ...
  // instance.elements.modalContent.addEventListener('transitionend', (e) => {
  //   e.preventDefault();
  //   e.stopPropagation();
  ...
  //     if (instance.elements.modal.classList.contains('resizing')) {
  //       instance.elements.modal.classList.remove('resizing');
  //     }
  //   }
  // }, false);
}
...
class BlsModal {
  ...
  */
  ...
  init(setting) {
    ...
    if (window.blsModal === undefined) window.blsModal = [];

    // buildBackdrop();
    const _totalModal = document.querySelectorAll('.bls-modal').length;
    els.map((x, index) => {
      const _f = window.blsModal.filter(y => y.element === x);
      if (_f.length === 0) {
        const obj = {};
        ...
        obj.elements = {
          btn: x,
          modal: obj.setting.type === 'inner' ? document.querySelector(x.dataset.target) : null,
        };
