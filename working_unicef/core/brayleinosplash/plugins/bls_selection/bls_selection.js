import './bls_selection.scss';
import '../../polyfill';
import { convertNodeListToArray, fireEvent, unWrap, getSelectors, } from '../../utils';

/* Private Method */
const eventOpen = new CustomEvent('blsSelection.Open', { bubbles: false, cancelable: false });
const eventClose = new CustomEvent('blsSelection.Close', { bubbles: false, cancelable: false });
...
const _openEventHandle = (e) => {
  const els = document.querySelectorAll('.bls-select.open');
...
const _clickOutsideHandle = (e) => {
  e.stopPropagation();
  const els = document.querySelectorAll('.bls-select');
...
function _bindEventChange(instance) {
  ...
  if (window.blsSelection !== undefined) {
    window.blsSelection.map((x) => {
...
function _bindEventOpen(instance) {
  // instance.elements.wrapper.addEventListener('blsSelection.Open', (e) => {
  //   const els = document.querySelectorAll('.bls-select.open');
  //   convertNodeListToArray(els).map(x => (x !== e.target) && (x.classList.remove('open')));
  // }, false);
  if (window.blsSelection !== undefined) {
    window.blsSelection.map((x) => {
      if (x.element === instance.elements.select) {
        instance.elements.wrapper.removeEventListener('blsSelection.Open', x.openEventHandle);
        instance.elements.wrapper.addEventListener('blsSelection.Open', x.openEventHandle);
      }
      return x;
    });
  } else {
    instance.elements.wrapper.addEventListener('blsSelection.Open', e => _openEventHandle(e));
  }
}

// function _bindEventClose(instance) {
//   instance.elements.wrapper.addEventListener('blsSelection.Close', () => {
  ...
  */
  constructor(setting) {
    const defaultSetting = {
      selector: 'select[data-bls-selection]',
      placeholder: '',
      events: {
        initialized() {},
        initializedAll() {},
  ...
  init(setting) {
    ...
    if (window.blsSelection === undefined) window.blsSelection = [];

    els.map((x) => {
      const _f = window.blsSelection.filter(y => y.element === x);
      if (_f.length === 0) {
        ...
        wrapper.classList.add('bls-select');
        x.parentNode.insertBefore(wrapper, x);
        wrapper.appendChild(x);

        // add UI element
        const blsText = document.createElement('div');
        const ul = document.createElement('ul');
        let ulString = '';

        blsText.className = `${x.className} btn btn-select bls-select-text`;
        blsText.innerText = obj.setting.placeholder;
        ...
        if (valueText.length) blsText.innerText = valueText.join();

        ul.innerHTML = ulString;
        ul.classList.add('bls-options');

        wrapper.appendChild(blsText);
